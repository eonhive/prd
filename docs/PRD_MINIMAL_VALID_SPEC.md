...
- The spec is published in the current flat `docs/` layout, even though the logical split may move later.
...
- `profile` MUST identify the intended PRD profile using a canonical machine-readable ID. Friendly UI labels do not satisfy this requirement. Profile identifier governance is defined in `PRD_PROFILE_REGISTRY.md`. Exact machine-readable registry publication remains future work.
