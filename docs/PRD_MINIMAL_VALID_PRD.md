...
It is a concise canonical extraction of the current minimal-package baseline already established in `PRD_MINIMAL_VALID_SPEC.md` and the accepted minimal-package decisions.

**Assumptions**

- `PRD_PROFILE_REGISTRY.md` governs profile identifiers and status, but a machine-readable public registry format is not defined yet.
- `PRD_CAPABILITY_MODEL.md` defines the current capability baseline, but fuller viewer conformance detail may still refine fallback behavior later.
...
Exact viewer conformance detail remains follow-on work beyond `PRD_CAPABILITY_MODEL.md`.
