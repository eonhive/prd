...
33. [x] Merged the corrective `0.1.1` changeset + registry-audit slice to `main`, let the `Release` workflow publish `0.1.1`, and verified all four public packages resolve on npm at `0.1.1`.
...
44. [ ] Run public launch QA for the Cloudflare production path: configure Cloudflare Pages to use repo root, build command `pnpm viewer:demo:build:cloudflare`, and output directory `apps/prd-viewer-web/dist`; then verify `https://prd.eonhive.com/`, `/viewer/`, `/docs/`, hosted sample archives, route refresh behavior, theme persistence, mobile layout, and manual `.prd` upload. Keep GitHub Pages as fallback until this passes.
